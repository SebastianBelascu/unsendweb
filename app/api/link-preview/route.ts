...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(target.href, {
      headers: {
        'user-agent':
          'Mozilla/5.0 (compatible; UnsendBot/1.0; +https://unsend.app)',
    ...
    if (!res.ok || !ct.includes('text/html')) return NextResponse.json(empty);
    ...
    // Intrinsic image size from the OG/Twitter dimension tags (YouTube,
    // Substack, most CMSs declare these). Lets the bubble paint the card at
    // the image's natural aspect ratio on the first frame — no crop, no jump.
    const dim = (v: string): number | undefined => {
      const n = parseInt(v, 10);
      return Number.isFinite(n) && n > 0 ? n : undefined;
    };
    const imageWidth = dim(
      meta('og:image:width') || meta('twitter:image:width'),
    );
    const imageHeight = dim(
      meta('og:image:height') || meta('twitter:image:height'),
    );
